...
class Chat2DeepSeek:
    def __init__(self, api_key, model = "deepseek-reasoner"):
        ...
        self.filename = "./log/{0}{1}_{2}{3}.json".format(str(random.randint(1, 1000)), "user_messages_{}".format(
            str(time.localtime().tm_mon) + '_' + str(time.localtime().tm_mday) + '_' + str(
                time.localtime().tm_hour) + '_' + str(time.localtime().tm_min)), str(time.localtime().tm_sec),
